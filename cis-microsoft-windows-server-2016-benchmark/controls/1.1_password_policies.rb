...
control 'çis-complexity-requirements-1.1.5' do
  impact 0.7
  title '1.1.5 Password must meet complexity requirements is set to Enabled'
  desc 'Password must meet complexity requirements is set to Enabled'
  describe security_policy do
    its('PasswordComplexity') { should be = 1 }
  end
end

control 'cis-password-reveribleencryption-1.1.6' do
  impact 0.7
