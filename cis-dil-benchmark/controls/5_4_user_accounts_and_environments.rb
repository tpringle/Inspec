...
#
# author:   Tim Pringle
# comments: Reworked dev-sec/cis-dil-benchmark to allow it to run on Inspec version 2.0.4.5
...
control 'cis-dil-benchmark-5.4.1.1' do
  ...
  shadow_files.each do |f|
    #for every shadow directory, get all the users, and for each one set it to objuser
    shadow(f).entries.each do |entry|
      if (entry.password)
        describe entry do
...
control 'cis-dil-benchmark-5.4.1.2' do
  ...
  shadow_files.each do |f|
    #for every shadow directory, get all the entries, and for each one set it to entry
    shadow(f).entries.each do |entry|
      if (entry.password)
        describe entry do
